...
paymentRouter.post('/', authenticate, authorizeRole(['cashier', 'admin', 'owner']), (req: AuthenticatedRequest, res) => {
  ...
  if (client_tx_id) {
    const existingPayment = db.prepare('SELECT p.*, r.receipt_number, r.content_json, r.verification_hash FROM payments p LEFT JOIN receipts r ON p.order_id = r.order_id WHERE p.client_tx_id = ?').get(client_tx_id) as any;
    if (existingPayment) {
  ...
  const existingReceipt = db.prepare('SELECT id, receipt_number, content_json, verification_hash FROM receipts WHERE order_id = ?').get(order_id) as any;
  if (existingReceipt) {
  ...
  const tx = db.transaction(() => {
    ...
    const alreadyDeducted = db.prepare(
      "SELECT COUNT(*) as count FROM inventory_movements WHERE reference_id = ? AND movement_type = 'ORDER_CONSUMPTION'"
    ...
    if (!alreadyDeducted || alreadyDeducted.count === 0) {
      const orderItems = db.prepare('SELECT menu_item_id, quantity FROM order_items WHERE order_id = ?').all(order_id) as any[];
      ...
      for (const oi of orderItems) {
        const ingredients = getRecipeItems.all(oi.menu_item_id) as any[];
        for (const ing of ingredients) {
          const totalDeduction = ing.quantity_required * oi.quantity;

          // Ensure stock record exists
          let currentStock = db.prepare('SELECT current_quantity FROM inventory_stock WHERE branch_id = ? AND ingredient_id = ?').get(order.branch_id, ing.ingredient_id) as any;
          if (!currentStock) {
            db.prepare('INSERT INTO inventory_stock (id, branch_id, ingredient_id, current_quantity) VALUES (?, ?, ?, 0)').run(
              `stk_${uuidv4().substring(0, 8)}`, order.branch_id, ing.ingredient_id
            );
          ...
          logMovementStmt.run(
            ...
            newQty,
            order_id,
            `Auto-deduction for Order #${order.order_number}`
          );

          // Check low stock threshold
          const ingMaster = db.prepare('SELECT min_stock_level, name FROM ingredients WHERE id = ?').get(ing.ingredient_id) as any;
...
paymentRouter.get('/receipt/:orderId', authenticate, (req: AuthenticatedRequest, res) => {
  const receipt = db.prepare('SELECT * FROM receipts WHERE order_id = ?').get(req.params.orderId) as any;
