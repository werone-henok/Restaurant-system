...
reportRouter.get('/dashboard', authenticate, authorizeRole(['admin', 'owner']), (req: AuthenticatedRequest, res) => {
  ...
  const rawDailyTrend = db.prepare(`
    ...
    ORDER BY date ASC
  `).all(trendStartDate, trendEndDate, ...branchParamsOrders) as Array<{ date: string; sales: number; orders: number }>;
...
reportRouter.get('/export/sales', authenticate, authorizeRole(['admin', 'owner']), (req: AuthenticatedRequest, res) => {
  ...
  `;
  const params: any[] = [];
  if (from) { sql += ` AND date(o.created_at, '${tzMod}') >= date(?)`; params.push(from); }
  if (to) { sql += ` AND date(o.created_at, '${tzMod}') <= date(?)`; params.push(to); }
