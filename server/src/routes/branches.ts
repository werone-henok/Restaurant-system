...
branchRouter.get('/', authenticate, (req: AuthenticatedRequest, res) => {
  const branches = db.prepare('SELECT * FROM branches WHERE is_active = 1 ORDER BY name ASC').all();
  res.json(branches);
});

// Get single branch
branchRouter.get('/:id', authenticate, (req: AuthenticatedRequest, res) => {
...
branchRouter.post('/', authenticate, authorizeRole(['admin', 'owner']), (req: AuthenticatedRequest, res) => {
...
branchRouter.put('/:id', authenticate, authorizeRole(['admin', 'owner']), (req: AuthenticatedRequest, res) => {
  const { name, city, address, phone, tax_number, vat_rate } = req.body;
  db.prepare(`
    UPDATE branches
...
branchRouter.delete('/:id', authenticate, authorizeRole(['admin', 'owner']), (req: AuthenticatedRequest, res) => {
