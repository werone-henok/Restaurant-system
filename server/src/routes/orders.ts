...
orderRouter.post('/', authenticate, (req: AuthenticatedRequest, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    `).run(
      ...
      resolvedOrderType, subtotal, taxAmount, totalAmount, special_notes || null
    );
...
orderRouter.post('/:id/confirm', authenticate, authorizeRole(['cashier', 'admin', 'owner']), (req: AuthenticatedRequest, res) => {
  ...
  let lowStockAlerts: { ingredient_id: string; name: string; unit: string; current: number; min: number }[] = [];
  try {
