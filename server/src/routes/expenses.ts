...
expenseRouter.get('/', authenticate, (req: AuthenticatedRequest, res) => {
...
// 2. Record Expense
expenseRouter.post('/', authenticate, authorizeRole(['admin', 'owner']), (req: AuthenticatedRequest, res) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    id, targetBranch, req.user!.id, category, amount,
    expense_date || new Date().toISOString().split('T')[0],
    description, receipt_photo_url || null, supplier_id || null, reference_number || null
  );
