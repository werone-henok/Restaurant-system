...
export function logAudit(params) {
    try {
        ...
        const lastEntry = db.prepare('SELECT entry_hash FROM audit_logs WHERE entry_hash IS NOT NULL ORDER BY rowid DESC LIMIT 1').get();
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, branchIdStr, params.userId || null, params.action, params.entityType, entityIdStr, detailsStr, prevHash, entryHash, createdAt);
        return;
    }
    catch (err) {
...
export function verifyAuditChain() {
    const entries = db.prepare('SELECT id, action, entity_type, entity_id, details, prev_hash, entry_hash, created_at FROM audit_logs WHERE entry_hash IS NOT NULL ORDER BY rowid ASC').all();
    if (entries.length === 0)
