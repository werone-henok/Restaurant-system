...
async function saveReceiptPhoto(dataUrlOrUrl) {
...
inventoryRouter.post('/receive', authenticate, authorizeRole(['storekeeper', 'admin', 'owner']), async (req, res) => {
    try {
        ...
        if (!finalSupplierId) {
            const anySup = db.prepare('SELECT id FROM suppliers LIMIT 1').get();
            if (anySup) {
      ...
      `);
            ...
            const sampleIng = db.prepare('SELECT category FROM ingredients WHERE id = ?').get(items[0].ingredient_id);
            const expenseCategory = (sampleIng?.category === 'Beverages' && items.every(it => {
                const c = db.prepare('SELECT category FROM ingredients WHERE id = ?').get(it.ingredient_id);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(expenseId, targetBranch, req.user.id, expenseCategory, totalCost, receivingDate, expenseDesc, receiptPhoto, finalSupplierId, invoice_number || poNumber);
        });
        tx();
        logAudit({
...
inventoryRouter.post('/waste', authenticate, authorizeRole(['storekeeper', 'chef', 'barista', 'admin', 'owner']), (req, res) => {
    ...
    // Get ingredient cost
    const ing = db.prepare('SELECT unit_cost, name FROM ingredients WHERE id = ?').get(ingredient_id);
...
inventoryRouter.get('/movements', authenticate, (req, res) => {
...
inventoryRouter.post('/ingredients', authenticate, authorizeRole(['storekeeper', 'admin', 'owner']), validate(createIngredientSchema), (req, res) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, name, name_amharic || null, category, sku || null, unit, unit_cost ?? 0, min_stock_level ?? 5, max_stock_level ?? 100, shelf_location || 'Main Store', photo_url || null, expiration_date || null);
...
inventoryRouter.put('/ingredients/:id', authenticate, authorizeRole(['storekeeper', 'admin', 'owner']), validate(updateIngredientSchema), (req, res) => {
    const { id } = req.params;
    ...
    SET name = COALESCE(?, name),
        ...
        expiration_date = ?
    WHERE id = ?
  `).run(name ?? existing.name, name_amharic !== undefined ? name_amharic : existing.name_amharic, category ?? existing.category, sku !== undefined ? sku : existing.sku, unit ?? existing.unit, unit_cost !== undefined ? unit_cost : existing.unit_cost, min_stock_level !== undefined ? min_stock_level : existing.min_stock_level, max_stock_level !== undefined ? max_stock_level : existing.max_stock_level, shelf_location ?? existing.shelf_location, photo_url !== undefined ? photo_url : existing.photo_url, expiration_date !== undefined ? expiration_date : existing.expiration_date, id);
    logAudit({
        branchId: req.user.branch_id,
        userId: req.user.id,
...
inventoryRouter.delete('/ingredients/:id', authenticate, authorizeRole(['admin', 'owner']), (req, res) => {
    const { id } = req.params;
    const existing = db.prepare('SELECT id, name FROM ingredients WHERE id = ?').get(id);
    if (!existing) {
        return res.status(404).json({ error: 'Ingredient not found' });
    }
...
inventoryRouter.get('/purchases', authenticate, (req, res) => {
    try {
        const branchId = req.query.branchId || req.user.branch_id;
