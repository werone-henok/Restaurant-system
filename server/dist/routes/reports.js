...
reportRouter.get('/dashboard', authenticate, authorizeRole(['admin', 'owner']), (req, res) => {
    const branchId = req.query.branchId; // if not provided or 'ALL', consolidated across all branches (Owner)
    ...
    const branchParamsOrders = [];
    const branchParamsExpenses = [];
    if (!isConsolidated) {
  ...
  `;
    ...
    const lowStockCount = db.prepare(lowStockQuery).get(...lowStockParams)?.count || 0;
...
reportRouter.get('/waiters', authenticate, authorizeRole(['admin', 'owner']), (req, res) => {
