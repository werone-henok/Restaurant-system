...
menuRouter.post('/categories', authenticate, authorizeRole(['admin', 'owner', 'chef']), (req, res) => {
    ...
    if (nextSortOrder === undefined || nextSortOrder === null) {
        const maxRow = db.prepare('SELECT COALESCE(MAX(sort_order), 0) + 1 as next_order FROM menu_categories').get();
        nextSortOrder = maxRow?.next_order || 1;
    }
    db.prepare(`
    INSERT INTO menu_categories (id, name, name_amharic, icon, sort_order, is_active)
    VALUES (?, ?, ?, ?, ?, 1)
  `).run(catId, name, name_amharic?.trim() || null, icon?.trim() || 'Utensils', nextSortOrder);
    logAudit({
...
menuRouter.put('/categories/:id', authenticate, authorizeRole(['admin', 'owner', 'chef']), (req, res) => {
    ...
    WHERE id = ?
  `).run(name ?? null, name_amharic ?? null, icon ?? null, sort_order !== undefined ? Number(sort_order) : null, is_active !== undefined ? (is_active ? 1 : 0) : null, req.params.id);
    logAudit({
        branchId: req.user.branch_id,
        userId: req.user.id,
...
menuRouter.delete('/categories/:id', authenticate, authorizeRole(['admin', 'owner', 'chef']), (req, res) => {
    const existing = db.prepare('SELECT * FROM menu_categories WHERE id = ?').get(req.params.id);
    if (!existing)
        return res.status(404).json({ error: 'Category not found' });
    // Check if any active menu items exist in this category
    const itemCount = db.prepare('SELECT COUNT(*) as count FROM menu_items WHERE category_id = ? AND deleted_at IS NULL').get(req.params.id);
...
menuRouter.get('/items', authenticate, (req, res) => {
  ...
  `);
    const enrichedItems = items.map(item => {
        const recipeRows = getRecipe.all(item.id);
        return {
            ...item,
...
menuRouter.post('/items', authenticate, authorizeRole(['admin', 'owner', 'chef']), (req, res) => {
...
menuRouter.put('/items/:id/recipe', authenticate, authorizeRole(['admin', 'owner', 'chef']), (req, res) => {
    ...
    const tx = db.transaction(() => {
        ...
        const existing = db.prepare('SELECT id FROM recipes WHERE menu_item_id = ?').get(menuItemId);
        let recipeId;
        if (existing) {
...
menuRouter.put('/items/:id', authenticate, authorizeRole(['admin', 'owner', 'chef']), (req, res) => {
    ...
    WHERE id = ?
  `).run(category_id ?? null, name ?? null, name_amharic ?? null, description ?? null, price !== undefined ? Number(price) : null, photo_url !== undefined ? photo_url : null, prep_time_minutes !== undefined ? Number(prep_time_minutes) : null, routing_destination ?? null, is_available !== undefined ? (is_available ? 1 : 0) : null, req.params.id);
    logAudit({
        branchId: req.user.branch_id,
        userId: req.user.id,
...
menuRouter.delete('/items/:id', authenticate, authorizeRole(['admin', 'owner', 'chef']), (req, res) => {
    const item = db.prepare('SELECT * FROM menu_items WHERE id = ?').get(req.params.id);
