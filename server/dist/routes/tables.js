...
tableRouter.patch('/:id/status', authenticate, (req, res) => {
...
tableRouter.post('/', authenticate, authorizeRole(['admin', 'owner']), (req, res) => {
...
tableRouter.put('/:id', authenticate, authorizeRole(['admin', 'owner']), (req, res) => {
...
tableRouter.delete('/:id', authenticate, authorizeRole(['admin', 'owner']), (req, res) => {
...
tableRouter.get('/sections', authenticate, (req, res) => {
    const branchId = req.query.branchId || req.user.branch_id;
    const rows = db.prepare('SELECT DISTINCT section FROM restaurant_tables WHERE branch_id = ? AND is_active = 1').all(branchId);
